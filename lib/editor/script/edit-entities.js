...
wm.EditEntities = mini.Class.subclass({
	...
	initialize: function( div ) {
		...
		this.menu = jQuery('#entityMenu');
		this.importEntityClass( WrappedEntityRepository );
		this.entityDefinitions = jQuery('#entityDefinitions');
		
		jQuery('#entityKey').bind( 'keydown', function(ev){
			if( ev.which == 13 ){ 
				jQuery('#entityValue').focus();
				return false;
			}
			return true;
		});
		jQuery('#entityValue').bind( 'keydown', this.setEntitySetting.bind(this) );
	},
	...
	selectEntity: function( entity ) {
		if( entity && entity != this.selectedEntity ) {
			...
			jQuery('#layerSettings').fadeOut(100,(function(){
				jQuery('#entitySettings').fadeOut(100,(function(){
					this.loadEntitySettings();
					jQuery('#entitySettings').fadeIn(100);
				}).bind(this));
			}).bind(this));
		} 
		else if( !entity ) {
			jQuery('#entitySettings').fadeOut(100);
			jQuery('#entityKey').blur();
			jQuery('#entityValue').blur();
		}
		
		this.selectedEntity = entity;
		jQuery('#entityKey').val('');
		jQuery('#entityValue').val('');
	},
	...
	moveSelectedEntity: function( x, y ) {
		...
		if( this.selectedEntity.pos.x != x || this.selectedEntity.pos.y != y ) {
			jQuery('#entityDefinitionPosX').text( x );
			jQuery('#entityDefinitionPosY').text( y );
	...
	loadEntitySettings: function(ent) {
		...
		jQuery('#entityClass').text( className );
		
		jQuery('.entityDefinition').bind( 'mouseup', this.selectEntitySetting );
	},
	...
	setEntitySetting: function( ev ) {
		...
		var key = jQuery('#entityKey').val();
		var value = jQuery('#entityValue').val();
	...
	selectEntitySetting: function( ev ) {
		jQuery('#entityKey').val( jQuery(this).children('.key').text() );
		jQuery('#entityValue').val( jQuery(this).children('.value').text() );
		jQuery('#entityValue').select();
	},
