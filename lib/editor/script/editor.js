...
jQuery.cookie = function(name, value) {
	var returnValue = $cookie.apply(jQuery, arguments);
...
wm.Weltmeister = Game.subclass({	
	...
	initialize: function() {
		...
		jQuery('#levelLoad').bind( 'click', Utils.Function.bind(this.showLoadDialog, this) );
		jQuery('#levelNew').bind( 'click', Utils.Function.bind(this.showNewDialog, this) );
		...
		jQuery('#levelSaveAs').bind( 'click', Utils.Function.bind(this.saveDialog.open, this.saveDialog) );
		jQuery('#levelSave').bind( 'click', Utils.Function.bind(this.saveQuick, this) );
		...
		this.entities = new EditEntities( jQuery('#layerEntities') );
		...
		jQuery(window).resize( Utils.Function.bind(this.resize, this) );
		jQuery(window).bind( 'keydown', Utils.Function.bind(this.uikeydown, this) );
		jQuery(window).bind( 'beforeunload', Utils.Function.bind(this.confirmClose, this) );
	
		jQuery('#buttonAddLayer').bind( 'click', Utils.Function.bind(this.addLayer, this) );
		jQuery('#buttonRemoveLayer').bind( 'click', Utils.Function.bind(this.deleteLayerDialog.open, this.deleteLayerDialog) );
		jQuery('#buttonSaveLayerSettings').bind( 'click', Utils.Function.bind(this.saveLayerSettings, this) );
		jQuery('#reloadImages').bind( 'click', function() { console.log("RELOAD IMAGES"); } ); // ig.Image.reloadCache );
		jQuery('#layerIsCollision').bind( 'change', Utils.Function.bind(this.toggleCollisionLayer, this) );
		
		jQuery('input#toggleSidebar').click(function() {
			jQuery('div#menu').slideToggle('fast');
			jQuery('input#toggleSidebar').toggleClass('active');
		});
		...
		if( wm.config.loadLastLevel ) {
			var name = jQuery.cookie('wmLastLevel');
			if( name ) {
				this.load( null, name );
			}
			else
	...
	*/
	...
	uikeydown: function( event ) {
		...
		if( key.match(/^\d$/) ) {
			...
			var name = jQuery('#layers div.layer:nth-child('+index+') span.name').text();
	...
	setWindowTitle: function() {
		...
		jQuery('span.headerTitle').text(this.fileName);
		jQuery('span.unsavedTitle').text(this.modified ? '*' : '');
	},
	...
	zoom: function( delta ) {
		...
		jQuery('#zoomIndicator').text( wm.config.view.zoom + 'x' ).stop(true,true).show().delay(300).fadeOut();
	...
	loadNew: function() {
		jQuery.cookie( 'wmLastLevel', null );
	...
	loadResponse: function( mapOrData ) {
		...
		jQuery.cookie( 'wmLastLevel', this.fileName );
		...
		*/
		...
		this.reorderLayers();
		jQuery('#layers').sortable('refresh');
	...
	reorderLayers: function( dir ) {
		...
		jQuery('#layers div.layer span.name').each((function( newIndex, span ){
			var name = jQuery(span).text();
	...
	updateLayerSettings: function( ) {
		jQuery('#layerName').val( this.activeLayer.name );
		jQuery('#layerTileset').val( this.activeLayer.tilesetName );
		jQuery('#layerTilesize').val( this.activeLayer.tilesize );
		jQuery('#layerWidth').val( this.activeLayer.movementScale.x );
		jQuery('#layerHeight').val( this.activeLayer.movementScale.y );
		jQuery('#layerPreRender').prop( 'checked', this.activeLayer.preRender );
		jQuery('#layerRepeat').prop( 'checked', this.activeLayer.repeat );
		jQuery('#layerLinkWithCollision').prop( 'checked', this.activeLayer.linkWithCollision );
		jQuery('#layerDistance').val( this.activeLayer.distance );
	},
	
	
	saveLayerSettings: function() {
		var isCollision = jQuery('#layerIsCollision').prop('checked');
		
		var newName = jQuery('#layerName').val();
		var newWidth = Math.floor(jQuery('#layerWidth').val());
		var newHeight = Math.floor(jQuery('#layerHeight').val());
		
		if( newWidth != this.activeLayer.width || newHeight != this.activeLayer.height ) {
			this.activeLayer.resize( newWidth, newHeight );
		}
		this.activeLayer.tilesize = Math.floor(jQuery('#layerTilesize').val());
		...
		*/
			/*
			var newTilesetName = jQuery('#layerTileset').val();
			if( newTilesetName != this.activeLayer.tilesetName ) {
				this.activeLayer.setTileset( newTilesetName );
			}
			*/
			this.activeLayer.linkWithCollision = jQuery('#layerLinkWithCollision').prop('checked');
			this.activeLayer.distance = jQuery('#layerDistance').val();
			this.activeLayer.repeat = jQuery('#layerRepeat').prop('checked');
			this.activeLayer.preRender = jQuery('#layerPreRender').prop('checked');
		//}
	...
	setActiveLayer: function( name ) {
		...
		//jQuery('#layerIsCollision').prop('checked', (name == 'collision') );
		
		//if( name == 'entities' ) { jQuery('#layerSettings').fadeOut(100); } else ...
		this.entities.selectEntity( null );
		//this.toggleCollisionLayer();
		jQuery('#entitySettings').fadeOut(100, (function(){
			jQuery('#layerSettings')
	...
	toggleCollisionLayer: function( ev ) {
		var isCollision = jQuery('#layerIsCollision').prop('checked');
		jQuery('#layerLinkWithCollision,#layerDistance,#layerPreRender,#layerRepeat,#layerName,#layerTileset')
...
wm.Weltmeister.getMaxHeight = function() {
	return jQuery(window).height() - jQuery('#headerMenu').height();
};
