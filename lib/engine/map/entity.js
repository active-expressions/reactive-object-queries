define([
	...
	var Entity = mini.Class.subclass({
		...
		applyDrawCallbacks: function(renderer) {
			for(var drawCallbackIndex=0; drawCallbackIndex < this.drawCallbacks.length; drawCallbackIndex++) {
	...
	Entity.prototype.toJson = function() {
		...
		if(this.drawCallbacks.length > 0) {
			json.drawCallbacks = [];
			for(var index=0; i < this.drawCallbacks.length; index++)
	...
	Entity.fromJson = function(json, world) {
		...
		if(typeof json.drawCallbacks !== "undefined")
			for(var index=0; index < json.drawCallbacks.length; index++)
