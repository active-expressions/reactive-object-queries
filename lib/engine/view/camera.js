define([
	...
	"jello",
	'num'
], function(require, Layer, Viewport, Jello, num) {
    var num = require("num");
	...
	var Camera = mini.Class.subclass({
		...
		getAABB: function(extentModifier) {
			var extent = this.viewport.extent.mulVector(extentModifier);
			return new Jello.AABB(
				this.viewport.point.sub(extent),
				this.viewport.point.add(extent)
			);
		}
	});
