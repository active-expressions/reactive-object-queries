define(['require', 'num'], function(require, num) {
	...
	var Canvas = mini.Class.subclass({
		initialize: function(canvasId) {
			...
			// TODO: wrap in a div
			if (jQuery("canvas#" + this.canvasId).length > 0) {
				this.$element = jQuery("canvas#" + this.canvasId);
			} else {
				this.$element = jQuery("<canvas></canvas>")
		...
		restore: function() {
			...
			if(this.callback) {
				jQuery(window).off("resize", this.callback);
		...
		screenFit: function(ratio) {
			...
			jQuery(window).resize(this.callback);
		...
		stretchOnResize: function() {
		    this.resize(jQuery(window).width(), jQuery(window).height());
		},
