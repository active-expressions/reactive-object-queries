define(function(require) {
	...
	// TODO: resolve parallel implementation to Raycast.intersectAABB
	Raycast.getPointOnAABB = function(aabb) {
		var result = this.intersectLineSegment(aabb.getTopLeft(), aabb.getTopRight());

		if(result.NumberOfSolutions() > 0) {
			return result.points[0];
		}

		result = this.intersectLineSegment(aabb.getTopRight(), aabb.getBottomRight());

		if(result.NumberOfSolutions() > 0) {
			return result.points[0];
		}

		result = this.intersectLineSegment(aabb.getBottomRight(), aabb.getBottomLeft());

		if(result.NumberOfSolutions() > 0) {
			return result.points[0];
		}

		result = this.intersectLineSegment(aabb.getBottomLeft(), aabb.getTopLeft());
