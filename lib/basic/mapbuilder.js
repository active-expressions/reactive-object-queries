define([
	...
	"editor/script/entity-dropdown",
	"external/activecollection/src/select",
    "external/activecollection/src/withlogging"
], function mapBuilderModule(
    require,
    num,
	ShapeBuilder,
	Utils,
	...
	var MapBuilder = {
        ...
        createExamples: function(layer, world) {
            // Track construction and destruction of Entity objects.
            // Log creation and destruction of Entities to setup a base set.
            withLogging.call(Entity);

            this.createEntityFinder(layer, world);
            //this.createOutOfScreenIndicators(layer, world);
        },
        createEntityFinder: function(layer, world) {
            ...
            matchingEntities.map(function(entity) {
				...
				item.addEventListener('click', entityFinder.focusCameraOn.bind(entityFinder, entity, layer));

                return item;
            })
                // When a new list item is created, append it to the list.
                .enter(function(item) {
                    entityFinder.div.append(item);
                })
        ...
        createOutOfScreenIndicators: function(layer, world) {
            // Find the point on the border of the screen that projects the out-of-screen Entity.
            // To do so, use the slightly modified camera viewport and perform a raycast on it.
            function findBorderDotFor(vec) {
                Raycast.origin.set(env.camera.viewport.point);
                Raycast.direction.set(vec.sub(env.camera.viewport.point));

                var extentModifier = new Vector2(0.98, 0.95);
                var cameraAABB = env.camera.getAABB().scaleVector(extentModifier);

                return Raycast.getPointOnAABB(cameraAABB);
            }

            select(Entity, function(entity) {
                // Check whether an Entity has a body, and thus a physical representation.
                // Then, check whether the Entity is out-of-screen.
                return entity.body && !env.renderer.isVisible(entity.body.getAABB());
            }).layer({
                // Define a layer to modify the behavior of selected Entities.
                debugDraw: function(debugDraw) {
                    // First, delegate to the original method or further layers.
                    cop.proceed(debugDraw);

                    // Then, get the point on the border of the screen and
                    // render a circle as indicator for the object's position.
                    var dotOnBorder = findBorderDotFor(this.body._derivedPosition);
                    debugDraw.drawCircle(dotOnBorder, 1, 'white', 1.0, 2, true, 'orange');
                }
            });
        },
		// Enhance given world with several bodies, etc. for testing purpose.
		"setUpMainLayer": function(layer, world) {
            this.createExamples(layer, world);
			...
			var pb = Jello.BodyFactory.createBluePrint()
				...
				.pointMasses(1.0)
				.translate(Vector2.Zero.copy())
				.translate(new Vector2(200, 50))
				.rotate(0)
			...
			for(var i = 0; i < 4; i++) {
				for(var j = 0; j < 4; j++) {
					...
					// create Entity
					var jellyCrate = new Entity("jellyBox").addToLayer(layer);
			...
			for (i = 0; i <= 2; i++) {
				for (j = 0; j <= 3; j++) {
					// Entity
					var fireflyEntity = new Entity("particle").addToLayer(layer);
			 ...
			 * testMapExtensions
			 */
			for(var extension=0; extension <  MapBuilder.testMapExtensions.length; extension++)
            ...
            // ===============================================================
            /*
             *  Spawn named entities
             */
			center = new Vector2(250, 50);
            loftBluePrint.translate(center).build();

			// Entity
			var guiElement = new Entity("CrateSpawner").addToLayer(layer);

			// Behaviour
			guiElement.setTrait(new Trait("showcase/spawnonclick", [layer]));
