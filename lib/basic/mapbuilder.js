define([
	...
	var MapBuilder = {
		...
		"setUpMainLayer": function(layer, world) {
			...
			// TODO: parallel implementation to Raycast.intersectAABB
			function getPointOnAABB(aabb) {
                var result = Raycast.intersectLineSegment(aabb.getTopLeft(), aabb.getTopRight());

                if(result.NumberOfSolutions() > 0) {
                    return result.points[0];
                }

                result = Raycast.intersectLineSegment(aabb.getTopRight(), aabb.getBottomRight());

                if(result.NumberOfSolutions() > 0) {
                    return result.points[0];
                }

                result = Raycast.intersectLineSegment(aabb.getBottomRight(), aabb.getBottomLeft());

                if(result.NumberOfSolutions() > 0) {
                    return result.points[0];
                }

                result = Raycast.intersectLineSegment(aabb.getBottomLeft(), aabb.getTopLeft());
			...
			function findBorderDotFor(vec) {
				Raycast.origin = env.camera.viewport.point;
				Raycast.direction = vec.sub(env.camera.viewport.point);

                var extentModifier = new Vector2(0.49, 0.475),
                    extent = env.camera.viewport.extent.mulVector(extentModifier),
                    cameraAABB = new Jello.AABB(
					    env.camera.viewport.point.sub(extent),
					    env.camera.viewport.point.add(extent)
    				);

				return getPointOnAABB(cameraAABB);
			}
			...
			select(Entity, function(entity) {
				...
				debugDraw: function(debugDraw) {
                    ...
                    debugDraw.configuration.setFillStyle('blue');
					debugDraw.drawCircle(dotOnBorder, 1, 'orange', 1.0, 2);
				}
			});
